			...
			if (keyvalueMap.size() != 0) {
				...
				for (int i = 0; i < keyvalueMap.size(); i++) {
					...
					if(entry.getKey().toString().endsWith("LIST")){
						...
						query.setParameterList(entry.getKey().toString(), paramList);
					}
					else{
...
public List<Object> findAndKillBySQLQuery(String queryString,
	...
	public List<?> fetchResultFromDB(Class className, String param, Object value, String projection) {
		logger.debug("Inside function fetchResultFromDB");
		Session session = factory.openSession();
		...
		logger.debug("returning results from fetchResultFromDB");
		return results;
	}	
}
