...
import com.PLATO.services.GenericCommandJenkinsJobService;
...
public class JenkinsDaoImpl implements JenkinsDao
{
	...
	private static final Logger logger=Logger.getLogger(JenkinsDaoImpl.class);
	...
	public ModuleJobsJenkins createModule(ModuleJobsJenkins module) throws Exception {
		...
		try{
			...
			jenkinsService.createJob(module.getDescription(),module.getJenkins_job_name(),module.getJenkins_job_id(),module.getModuleJobsJenkinsParametersList());
			session.flush();
	...
	public ModuleJobsJenkins updateModule(ModuleJobsJenkins module) throws Exception {
		logger.info("Inside function updateModule");
		logger.debug("Received parameter ModuleJobsJenkins with name as :"+module.getJenkins_job_name());
		Transaction tx=null;
		JenkinsServices jenkinsService=new JenkinsServices();
		Session session = factory.openSession();
		...
		try{
			...
			session.saveOrUpdate(module);
			//create job in jenkins
			jenkinsService.updateJob(module.getDescription(),module.getJenkins_job_name(),module.getJenkins_job_id(),module.getModuleJobsJenkinsParametersList());
			session.flush();
			//commit the transaction if creation of job successful in both database and jenkins
			tx.commit();
			logger.info("Schedulee job in both database and jenkins and Committed the transaction");
		}
		//roll back transaction if there is exception. Thus entry is not created in database if there is error while creating job in jenkins
		catch(Exception e)
		{
	...
	public void createSubModule(ModuleJobsJenkins module,Set<ModuleSubJobsJenkins> moduleSetForPostBuild) throws Exception {
		...
		try{
			...
			for(ModuleSubJobsJenkins subJob:subJobList)
			{
				...
				if(!(subJob.getSubjob_name().equals(module.getJenkins_job_name()))) 
				{
					...
					{
						GenericCommandJenkinsJobService jenkinsJobService=new GenericCommandJenkinsJobService();
						Document doc=jenkinsJobService.createGenericDoc(subJob,"");
						jenkinsJobService.writeSubJobInJenkins(doc, subJob);
					...
					}else if(subJob.getTool_name().equalsIgnoreCase("JUNIT")) {
						//jenkinsService.createSubJob(subJob);
					}else if(subJob.getTool_name().equalsIgnoreCase("BUILD")){
						...
						GenericCommandJenkinsJobService jenkinsJobService=new GenericCommandJenkinsJobService();
						Document doc=jenkinsJobService.createGenericDoc(subJob,"RecentReport.xlsx");
						jenkinsJobService.writeSubJobInJenkins(doc, subJob);
	
					}else if(subJob.getTool_name().equalsIgnoreCase("rahul")) {
						GenericCommandJenkinsJobService jenkinsJobService=new GenericCommandJenkinsJobService();
						Document doc=jenkinsJobService.createGenericDoc(subJob,"rahul.txt");
						jenkinsJobService.writeSubJobInJenkins(doc, subJob);
					}else {
						GenericCommandJenkinsJobService jenkinsJobService=new GenericCommandJenkinsJobService();
						Document doc=jenkinsJobService.createGenericDoc(subJob,"");
			...
			//if(subJobOrder.isEmpty() || subJobOrder==null){
			String finalSubJobString="";
			
			
			String[] subJobArr=subJobOrder.split(",");
			
			for(int s=0;s<subJobArr.length;s++){
				if(subJobArr[s].equalsIgnoreCase(module.getJenkins_job_name())){
						subJobArr[s]="";
						
					
				}else{
					if(s==subJobArr.length-1){
						finalSubJobString=finalSubJobString+subJobArr[s];
					}else{
						finalSubJobString=finalSubJobString+subJobArr[s]+",";
					}
			...
			/*
			if(subJobOrder.contains(module.getJenkins_job_name()+","))
				subJobOrder=subJobOrder.replace(module.getJenkins_job_name()+",", "");
			if(subJobOrder.contains(","+module.getJenkins_job_name()))
				subJobOrder=subJobOrder.replace(","+module.getJenkins_job_name(), "");
			if(subJobOrder.equalsIgnoreCase(module.getJenkins_job_name()))
				subJobOrder=subJobOrder.replace(module.getJenkins_job_name(), "");
			
			*/
			System.out.println("finalSubJobString ="+finalSubJobString);
			if(!finalSubJobString.equals("")){
			if(finalSubJobString.charAt(finalSubJobString.length()-1)==','){
				finalSubJobString=finalSubJobString.substring(0, finalSubJobString.length()-1);
				System.out.println(finalSubJobString+"newwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww");
			}
			}
			module.setModule_subjobs_order(finalSubJobString);
	...
	public void createSubModule(ModuleJobsJenkins module) throws Exception {
		...
		try{
			...
			*/
			System.out.println(finalSubJobString);
			if(!finalSubJobString.equals("")){
			if(finalSubJobString.charAt(finalSubJobString.length()-1)==','){
				finalSubJobString=finalSubJobString.substring(0, finalSubJobString.length()-1);
				System.out.println(finalSubJobString+"newwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww");
			...
			jenkinsService.setChildProjectsInMainJob(module.getModuleSubJobsJenkins(),module.getJenkins_job_name());
	...
	public void updateSubModule(ModuleSubJobsJenkins subModule) throws Exception
	{
		...
		try
		{
			...
			//session.saveOrUpdate(subModule);
			//System.out.println("name "+subModule.getSubjob_name());
			//System.out.println("id "+subModule.getSubjob_id());
			session.saveOrUpdate(subModule);
			session.flush();
			...
			if(subModule.getTool_name().equalsIgnoreCase("FAST"))
			{
				...
				GenericCommandJenkinsJobService jenkinsJobService=new GenericCommandJenkinsJobService();
				Document doc=jenkinsJobService.createUpdatedGenericDoc(subModule, "");
				jenkinsJobService.writeUpdatedSubJobInJenkins(doc, subModule);
			...
			}else if(subModule.getTool_name().equalsIgnoreCase("BUILD")){
